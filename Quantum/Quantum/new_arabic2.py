...
class EntanglionGame:
    ...
    def __init__(self, root):
        ...
        self.quantum_components = ["تصحيح الخطأ الكمي", "كيوبت الفيزيائية", "بوابات الكم", "التدريع المغناطيسي",
                                   "ربط الكيوبت", "البرمجة الكمومية", "ثلاجة التخفيف", "البنية التحتية للتحكم"]
        self.detection_rate = 1
        self.ship_locations = {}
        self.engine_deck = ["X", "H", "SWAP", "CNOT", "PROBE"]
        self.quantum_event_deck = ["Bit Flip Error", "انهيار وظيفة الموجة", "شرودنغر","المراوغة الكمومية"]
    ...
    def initialize_ship_locations(self):
        ...
        self.GLabel_333["text"] = "اللاعب 1: اضغط لرمي النرد المركزي ..."
        ...
        self.GLabel_333["text"] = "اللاعب 2: اضغط لرمي النرد المركزي ..."
        self.GButton_732.wait_variable(but_var)
    ...
    def roll_dice1(self):
        ...
        self.GLabel_333["text"] = "النرد:" + str(dice_roll)
        self.GButton_732.wait_variable(but_var)
        return dice_roll
    ...
    def draw_engine_cards(self, player, num_cards):
        ...
        for _ in range(num_cards):
            if self.engine_card_pile:
                ...
                self.GLabel_333["text"] = "لا يوجد المزيد من بطاقات المحرك المتبقية في الكومة."
                self.GButton_732.wait_variable(but_var)
                break
        return cards
    ...
    def perform_retrieval_mission(self, player):
        ...
        if outcome > self.detection_rate:
            ...
            self.GLabel_333["text"] = "تم استرداد المكون:" + component + "\nوضع المكون على متن سفينة الفضاء ..."
            self.GButton_732.wait_variable(but_var)
        else:
            #print("Away team was detected by ground defenses!")
            self.GLabel_333["text"] = "تم رصد الفريق الضيف بالدفاعات الأرضية!"
            self.GButton_732.wait_variable(but_var)
            self.detection_rate += 1
            #print("Detection rate increased to:", self.detection_rate)
            self.GLabel_333["text"] = "زيادة معدل الكشف إلى:" + str(self.detection_rate)
            self.GButton_732.wait_variable(but_var)
            
    def roll_dice(self):
        #input("Press Enter to roll the Entanglion dice...")
        self.GLabel_333["text"] = "اضغط لرمي نرد Entanglion ..."
        self.GButton_732.wait_variable(but_var)
        dice_roll = random.randint(1, 8)
        #print("Dice roll:", dice_roll)
        self.GLabel_333["text"] = "لفة النرد:" + str(dice_roll)
        self.GButton_732.wait_variable(but_var)
        return dice_roll
    ...
    def play_game(self):
        ...
        while True:
            ...
            self.GLabel_333["text"] = "اللاعب الحالي:" + self.current_player
            self.GButton_732.wait_variable(but_var)
            self.show_game_status()
            #action = input("Choose an action (navigate/exchange/retrieve/event): ")
            self.GLabel_333["text"] = "اختر إجراء (تنقل / تبادل / استرجاع / حدث): "
            self.GButton_732.wait_variable(but_var)
            ...
            if action == "تنقل":
                ...
                if self.are_players_in_entanglion():
                    ...
                    self.GLabel_333["text"] = "لا يمكن الاسترداد خارج Entanglion."
                    self.GButton_732.wait_variable(but_var)
    ...
    def navigate(self):
        ...
        self.GLabel_333["text"] = "لاعب " + player + ", التنقل ..."
        self.GButton_732.wait_variable(but_var)
        #card = input("Choose an engine card from your hand: ")
        self.GLabel_333["text"] = "اختر بطاقة محرك من يدك: "
        self.GButton_732.wait_variable(but_var)
        ...
        if card in self.engine_control[player]:
            ...
            self.GLabel_333["text"] = "لعبت بطاقة المحرك:" + card
            self.GButton_732.wait_variable(but_var)
            ...
            if card == "X":
                if self.ship_locations[player]["Galaxy"] == "Centarious" and self.ship_locations[player]["Planet"] in ["ZERO", "ONE"]:
                    ...
                    if self.ship_locations[other_player]["Galaxy"] == "Superious" and self.ship_locations[other_player]["Planet"] == "PLUS":
                        ...
                        self.GLabel_333["text"] = "لاعب " + player + " لا يمكن التنقل باستخدام بطاقة X في الوقت الحالي."
                        self.GButton_732.wait_variable(but_var)
                ...
                if self.ship_locations[player]["Galaxy"] == "Superious" and self.ship_locations[player]["Planet"] in ["PLUS", "MINUS"]:
                    ...
                    self.GLabel_333["text"] = "لاعب " + player + " لا يمكن التنقل بالبطاقة H في الوقت الحالي."
                    self.GButton_732.wait_variable(but_var)
            elif card == "CNOT":
                if self.ship_locations[player]["Galaxy"] == "Centarious" and self.ship_locations[player]["Planet"] == "ZERO":
                    ...
                    if self.ship_locations[other_player]["Galaxy"] == "Centarious" and self.ship_locations[other_player]["Planet"] == "ONE":
                        ...
                        self.GLabel_333["text"] = "لاعب " + player + " لا يمكن التنقل باستخدام بطاقة CNOT في الوقت الحالي."
                        self.GButton_732.wait_variable(but_var)
                elif self.ship_locations[player]["Galaxy"] == "Centarious" and self.ship_locations[player]["Planet"] == "ONE":
                    other_player = self.get_other_player(player)
                ...
                if self.ship_locations[player]["Galaxy"] == "Entanglion" and self.ship_locations[player]["Planet"] == "OMEGA_THREE":
                    ...
                    self.GLabel_333["text"] = "لاعب " + player + " لا يمكن التنقل باستخدام بطاقة SWAP في الوقت الحالي."
                    self.GButton_732.wait_variable(but_var)
            else:
                #print(f"Invalid engine card: {card}")
                self.GLabel_333["text"] = "بطاقة المحرك غير صالحة: " + card
                self.GButton_732.wait_variable(but_var)
            #print()
        ...
        if new_card:
            ...
            self.GLabel_333["text"] = "بطاقة جديدة: " + new_card[0]
            self.GButton_732.wait_variable(but_var)
    ...
    def update_ship_location(self, player, from_galaxy, from_planet, to_galaxy, to_planet):
        ...
        self.GLabel_333["text"] = "لاعب " + player + "انتقلت السفينة من" + from_galaxy + " " + from_planet + " الى " + to_galaxy + " " + to_planet + "."
        self.GButton_732.wait_variable(but_var)
    ...
    def swap_ships(self):
        ...
        #print("Ships swapped.")
        self.GLabel_333["text"] = "تبادل السفن."
        self.GButton_732.wait_variable(but_var)

    def exchange(self):
        player = self.current_player
        #print(f"Player {player}, exchanging...")
        self.GLabel_333["text"] = "لاعب " + player + ", تبادل ..."
        self.GButton_732.wait_variable(but_var)
        #card = input("Choose an engine card from your hand to discard: ")
        self.GLabel_333["text"] = "اختر بطاقة محرك من يدك للتخلص منها:"
        self.GButton_732.wait_variable(but_var)
        card = self.GLineEdit_751.get()
        self.GLineEdit_751.delete(0, 'end')
        if card in self.engine_control[player]:
            ...
            self.GLabel_333["text"] = "تم تجاهل بطاقة المحرك: " + card
            self.GButton_732.wait_variable(but_var)
            new_card = self.draw_engine_cards(player, 1)
            if new_card:
                self.engine_control[player].append(new_card[0])
